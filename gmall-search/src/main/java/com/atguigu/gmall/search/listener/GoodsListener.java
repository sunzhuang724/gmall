    ...
    public void listener(Long spuId, Channel channel, Message message) throws IOException {
        try {
            ...
            if (!CollectionUtils.isEmpty(skuEntities)){
                List<Goods> goodsList = skuEntities.stream().map(skuEntity -> {
                    ...
                    ResponseVo<List<SkuAttrValueEntity>> skuAttrValueResponseVo =
                            this.pmsClient.querySearchSkuAttrValueBySkuIdAndCid(skuEntity.getId(), skuEntity.getCatagoryId());
                    ...
                    ResponseVo<List<SpuAttrValueEntity>> spuAttrValueResponseVo =
                            this.pmsClient.querySearchSpuAttrValueBySpuIdAndCid(skuEntity.getSpuId(), skuEntity.getCatagoryId());
            ...
            if (message.getMessageProperties().getRedelivered()){
                // 当requeue参数是false时，消息将会被认定为死信消息。如果当前队列没有绑定死信队列时，该消息将会丢失
