...
public class CartService {
    ...
    private static final String KEY_PRIFIX = "cart:info:";
    ...
    public void addCart(Cart cart) {
        ...
        // 通过外层的key获取内层的map结构
        BoundHashOperations<String, Object, Object> hashOps = this.redisTemplate.boundHashOps(KEY_PRIFIX + userId);
    ...
    public List<Cart> queryCarts() {
        ...
        // 2.查询未登录的购物车内层map
        BoundHashOperations<String, Object, Object> unLoginHashOps = this.redisTemplate.boundHashOps(KEY_PRIFIX + userKey);
        ...
        if (!CollectionUtils.isEmpty(unloginCartJsons)){
            unLoginCarts = unloginCartJsons.stream().map(cartJson -> {
                ...
                System.out.println("134134"+this.redisTemplate.opsForValue().get(PRICE_PREFIX + cart.getSkuId()));
        ...
        // 4.获取登录状态的购物车内层map
        BoundHashOperations<String, Object, Object> loginHashOps = this.redisTemplate.boundHashOps(KEY_PRIFIX + userId);
        ...
        if (!CollectionUtils.isEmpty(unLoginCarts)){
            ...
            // 6.删除未登录的购物车
            this.redisTemplate.delete(KEY_PRIFIX + userKey);
        ...
        if (!CollectionUtils.isEmpty(loginCartJsons)){
            return loginCartJsons.stream().map(cartJson -> {
                Cart cart = JSON.parseObject(cartJson.toString(), Cart.class);
                System.out.println("180180"+this.redisTemplate.opsForValue().get(PRICE_PREFIX + cart.getSkuId()));
                cart.setCurrentPrice(new BigDecimal(this.redisTemplate.opsForValue().get(PRICE_PREFIX + cart.getSkuId())));
                return cart;
            }).collect(Collectors.toList());
        }
